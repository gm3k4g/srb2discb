...
async fn start_bot() {
    ...
    let token = DISCORD_TOKEN; //env::var("DISCORD_TOKEN").expect("Expected a token in the environment");
...
To print this message again, you can either call {NAME} with no arguments, or call it with the `-h` or `--help` arguments.
...
ARGS:
\t<CLI ARGS>
\t\tA list of parameters that SRB2 can process before the game starts.
...
\t\t  `{NAME} -p -dedicated -port 5029 -room 38`

\t<CLI ARGS>
\t\tCommandline arguments for SRB2. This can be one argument or many, it doesn't matter. It's advised to visit the SRB2 wiki ({CLI}) to look at what commandline arguments you can provide to SRB2.


OPTIONS:
\t-p
\t\tThis option is used to indicate that everything following this option will be paramters to give to SRB2 before it starts.

\t-h, --help
\t\tOnly displays the help message.
");
